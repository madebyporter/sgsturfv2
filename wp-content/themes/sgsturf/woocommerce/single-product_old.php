...
if ($group_product->is_type('grouped')):
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    productCards.forEach(function (productCard) {
      ...
      // Function to hide the "View Specs" button
      const hideViewSpecsButton = function () {
        cardBottom.classList.remove('specs-active');
        const buttonText = specsButton.querySelector('.spec-label');
        buttonText.textContent = 'View Specs';
      };

      // Initial hide before the page loads
      hideViewSpecsButton();

      specsButton.addEventListener('click', function (event) {
        event.stopPropagation();

        const productSpecs = productCard.querySelector('.product-specs');

        // Check if any element with the class 'font-bold' within '.product-specs' contains the text 'n/a'
        const naPresent = Array.from(productSpecs.querySelectorAll('.font-bold')).some(el => el.textContent.trim().toLowerCase() === 'n/a');
        ...
        if (!naPresent) {
          ...
          // If "n/a" is present, prevent toggling and hide the button
          hideViewSpecsButton();
          // You can also choose not to toggle the visibility or take other actions.
        }
      });
    });
  });
