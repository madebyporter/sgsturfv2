...
if ($query->have_posts()) {
                ...
                <div class="pattern-card mr-20 flex gap-8 justify-start content-start">
                  <!-- Group Sub Product Loop Start -->
									...
									foreach ($subproducts as $subproduct_id) {
										...
										$subproduct_categories = wp_get_post_terms($subproduct_id, 'product_cat', array('fields' => 'names')); // Get product categories for the subproduct
  ...
  <?php
  $product_gallery_images = $group_product->get_gallery_image_ids(); // Get gallery image IDs for the product
